...
# IMPORTANTE
#
# Ahora intentamos generar la predicción durante los
# últimos 2 minutos del contrato actual.
#
# Esto da más margen si Android pausa brevemente la app.
# ============================================================
...
# ============================================================
# BUSCAR SIGUIENTE CONTRATO REAL
# ============================================================
...
def guardar_prediccion(
    ...
    existente = buscar_registro_por_base(
        ticker_base
    )
...
#
# Cuando vuelves a la app después de estar viendo videos,
# intenta recuperar automáticamente todo lo que quedó
# pendiente.
# ============================================================
...
def actualizar_pendientes():
    ...
    for registro in historial:
        ...
        # ====================================================
        # 1. IDENTIFICAR CONTRATO REAL PREDICHO
        # ====================================================
        ...
        # ====================================================
        # GUARDAR TICKER Y TARGET
        # AUNQUE TODAVÍA NO HAYA TERMINADO.
        # ====================================================
...
# ============================================================
# RECUPERAR ÚLTIMA PREDICCIÓN
#
# No depende del contrato actual.
# ============================================================
...
st.caption(
    "Predicción automática del siguiente contrato "
    "BTC 15M de Kalshi."
)
...
#
# SE EJECUTA AL ABRIR / REGRESAR A LA APP.
# ============================================================
...
if actual is None:
    ...
    st.info(
        "Cuando regreses a la aplicación se volverá "
...
# IMPORTANTE:
#
# Ya NO borramos la última predicción.
# Solo actualizamos cuál es el contrato actual.
# ============================================================
...
if (
    ...
    # prediccion = None
    #
    # porque ahora queremos conservar
    # la última predicción visible.
...
# ============================================================
# REVISAR SI YA EXISTE PREDICCIÓN
# ============================================================

registro_actual = buscar_registro_por_base(
    ticker_actual
)
...
if registro_actual is not None:
    ...
    # Recuperamos la predicción desde historial.
...
if (
    st.session_state.prediccion is not None
):
    ...
    st.info(
        "La predicción automática se realizará "
        "durante los últimos 2 minutos del contrato. "
...
# ÚLTIMA PREDICCIÓN GUARDADA
#
# ESTA SECCIÓN ES LA MÁS IMPORTANTE PARA LO QUE TE PASÓ.
#
# Aunque ya haya cambiado el contrato actual,
# la última predicción permanece visible.
# ============================================================
...
# ============================================================
# CONTRATO PREDICHO DEL CONTRATO ACTUAL
# ============================================================
...
# ACTUALIZAR PENDIENTES
#
# Una segunda comprobación antes de mostrar estadísticas.
# ============================================================
...
st.caption(
    "El sistema analiza el contrato BTC 15M actualmente "
    "vigente. Durante los últimos 2 minutos utiliza el "
    "Target de ese contrato y los indicadores recientes "
    "de BTC para generar una predicción sobre el SIGUIENTE "
    "contrato. La predicción se guarda inmediatamente en "
    "el historial. Si la aplicación queda en segundo plano "
    "por Android, al regresar se revisan automáticamente "
    "los contratos pendientes. El sistema obtiene el "
    "contrato real predicho, su Target y su Expiration Value. "
    "Si Expiration Value > Target, Kalshi termina ARRIBA. "
    "Si Expiration Value < Target, termina ABAJO. "
    "Después compara ese resultado con la predicción y "
    "marca ACIERTO o FALLÓ. El historial no se reinicia "
